...
import { Menu, LogIn, LogOut, Loader2, AlertCircle, Search, Flame, Clock, PenTool, MoreHorizontal, ArrowRight, ChevronLeft, ChevronRight, Heart, X, CheckCircle } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleCreateTopic = async (e: React.FormEvent) => {
    ...
    try { const response = await fetch('/api/topics', { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify(newTopic) }); if (response.ok) { setIsCreateModalOpen(false); fetchTopics(); } } catch (error) { console.error("Create topic failed", error); }
  };
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-2 sm:gap-6 mb-8 border-b-2 border-dashed border-gray-300 pb-1">
                <button
                    ...
                    className={`flex items-center gap-2 pb-2 text-xl sm:text-2xl font-hand font-bold transition-all border-b-4 rounded-t-lg px-4 ${
                        homeTab === 'popular'
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <header className="h-16 bg-[#faf8f6] border-b border-gray-300 sticky top-0 z-40 px-4 flex items-center justify-between shadow-sm bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]">
        ...
        <div className="flex items-center gap-3">
          {user ? (
            <div className="flex items-center gap-3">
               <span className="font-hand font-bold hidden md:block">{user.username}</span>
      ...
      <div className="flex flex-1 relative">
        <Sidebar viewMode={viewMode} selectedCategory={selectedCategory} onSelectView={handleSelectView} isOpen={isSidebarOpen} onCloseMobile={() => setIsSidebarOpen(false)} />
